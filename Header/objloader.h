...
// 不支持材质的版本
inline unordered_map<string, Material> LoadMtlLegacy(string path) {
...
inline unordered_map<string, Material> LoadMtl(string path) {
	...
	for (uint i = 0; i < names.size(); i++) {
		if (textureCollectionDiffuse.find(names.at(i)) != textureCollectionDiffuse.end()) {
			...
			TextureManager::GetInstance().Add(names.at(i)+"BaseColorMap",textureCollectionDiffuse[names.at(i)]);
		}
		else{
		...
		if (textureCollectionNormal.find(names.at(i)) != textureCollectionNormal.end()) {
			...
			TextureManager::GetInstance().Add(names.at(i)+"NormalMap",textureCollectionNormal[names.at(i)]);
		}
		else{
		...
		if (textureCollectionARM.find(names.at(i)) != textureCollectionARM.end()) {
			...
			TextureManager::GetInstance().Add(names.at(i)+"ARMMap",textureCollectionARM[names.at(i)]);
		}
		else{
